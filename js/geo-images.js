/* ── Geotagged Photographs (Deposit 4) ── */
let geoImagesLayer;
var PHOTO_LIST = [];
...
fetch('data/geotag_metadata.json')
    ...
    .then(images => {
        ...
        geoImagesLayer = L.featureGroup();
        const ref = images[0];
        const count = images.length;

        const marker = L.marker([ref.latitude, ref.longitude], {
            icon: L.divIcon({
                className: 'photo-marker-icon',
                html: `<svg viewBox="0 0 24 24" width="24" height="24"><circle cx="12" cy="12" r="10" fill="#2c5f8a" stroke="#fff" stroke-width="2"/><path d="M16 9.5H8a1 1 0 0 0-1 1v4a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-4a1 1 0 0 0-1-1z" fill="none" stroke="#fff" stroke-width="1.2"/><circle cx="12" cy="12" r="2" fill="none" stroke="#fff" stroke-width="1.2"/><path d="M15 9.5l-1-1.5h-4L9 9.5" fill="none" stroke="#fff" stroke-width="1.2"/></svg>${count > 1 ? `<span class="marker-count">${count}</span>` : ''}`,
                iconSize: [28, 28],
                iconAnchor: [14, 14],
                popupAnchor: [0, -16]
            })
        ...
        marker.bindPopup(buildGalleryPopup(images));
        geoImagesLayer.addLayer(marker);

        if (typeof layerMap !== 'undefined') {
            layerMap.dep4photos = geoImagesLayer;
